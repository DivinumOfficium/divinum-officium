...
sub tfertable {
  ...
  $fname =
      ($version =~ /Newcal/i) ? "TrNewcal$kyearo"
    : ($version =~ /1955|1960/) ? "Tr1960$kyearo"
    : ($version =~ /1570/i) ? "Tr1570$kyearo"
    : ($version =~ /1910/) ? "Tr1910$kyearo"
    : ($version =~ /Monastic/i) ? "TrM$kyearo"
    : "TrDA$kyearo";
  $fname1 = $fname;
  $fname1 =~ s/^Tr/Str/;
  ...
  if (do_write("$datafolder/Latin/Tabulae/$fname.txt", @tfer_out)) {
    ...
    $error .= "$datafolder/Latin/Tabulae/$fname.txt cannot open for output<BR>\n";
  }
  ...
  if (do_write("$datafolder/Latin/Tabulae/$fname1.txt", @scriptfer_out)) {
    ...
    $error .= "$datafolder/Latin/Tabulae/$fname1.txt cannot open for output<BR>\n";
  }
}
...
sub collect_arrays {
  ...
  if (my @a = do_read("$datafolder/Latin/Tabulae/K$kalendarname.txt")) {
    ...
    error("$datafolder/Latin/Tabulae/$kalendarname.txt cannot open");
  }
  ...
  for ($tmonth = 0; $tmonth < @monthlength; $tmonth++) {
    ...
    for (my $kday = 1; $kday <= $monthlength[$tmonth]; $kday++) {
      ...
      if ($dayname[0] =~ /Epi1/i && $dayofweek == 0) {
        if ($version =~ /(Trident|Monastic)/i) {
          push(@tfer, sprintf("%02i-%02i=$tempname/Epi1-0a", $kmonth, $kday));
        } elsif ($version =~ /Divino|1955/i) {
          push(@scriptfer, sprintf("%02i-%02i=Epi1-0a", $kmp1, $kdp1));
        } elsif ($kday == 13) {
      ...
      if ( $version !~ /(Monastic|1960)/i
        && !$tempflag
        && $oldrank > $rank
        && exists($w{'Hymnus Vespera'})
        && $w{'Hymnus Vespera'} !~ /\@/
        && exists($w{'Hymnus Matutinum'})
        && $w{'Hymnus Matutinum'} !~ /\@/)
      {
      ...
      if (
        ...
        && $dayofweek == 0
        && $sday !~ /v$/
        && $dayname[0] !~ /(Adv[2-4]|Quad[1-7])/i
        && $sday !~ /01\-05/
        && ( $saint{Rank} =~ /Vigilia/i
          || exists($saint{'Oratio Vigilia'})
          || $kmonth == 10 && $kday == 31)
      ) {
        my $km = $kmonth;
        my $kd = $kday - 1;
      ...
      if ($version !~ /Trident/i && $kmonth == 10 && $kday > 24 && $dayofweek == 0) {
        if ($version =~ /1960/) {
          push(@tfer, sprintf("%02i-%02i=10-DUr", $kmonth, $kday));
        } else {
      ...
      if ($version !~ /1960/) {
        if ($dayofweek == 0) {
          if (@impeded) {
            if (!@free) {
              push(@scriptfer, "unsolved=@impeded");
            } else {
              my $line = '';
        ...
        if ($kmonth == 9 && $tempora{Rank} =~ /(III\. Septemb|Quat)/i) {
          if (exists($saint{Lectio1}) && $tempora{Rank} !~ /Quat/i && ($tempora{Rank} !~ /Dominica/i || $srank[2] >= 5))
          {
            ...
            && (($tempora{Rank} =~ /Quat/i && $srank[2] >= 2) || $tempora{Rank} !~ /Quat/i)
            && @qimpeded)
          {
        ...
        if (
          ...
          && $saint{Rank} !~ /;;ex C9/i
          && !exists($saint{Lectio1})
          && !((($version !~ /Trident/i && $saint{Rank} =~ /;;ex C/) || ($version =~ /Trident/i && $srank[2] >= 3))))
        {
          if (@impeded) {
            my $line = '';
            foreach (@impeded) { $line .= "$_~"; }
            ...
            push(@free, sprintf("%02i-%02i", $kmonth, $kday));
          }
        }
        ...
        if ($tempora{Rank} =~ /Nov/i || $macc2num > 2) {
          $macc2flag = 0;

        } elsif ($macc2flag) {
          if ($dayofweek == 0) {
            if ($version =~ /trident/i && (transfered($sday) || !exists($saint{Lectio1}))) { $macc2num++; }
          } elsif (!exists($saint{Lectio1})) {
      ...
      if ($day == 13 && $tname =~ /Epi1/i && $version !~ /1955|1960/i) { push(@tfer, "01-12=Tempora/Epi1-0"); next; }

      if (
           $trank[2] >= 6
        && $srank[2] >= 6
        && ( $saint{Rule} !~ /Festum Domini/i
          || $tempora{Rule} !~ /Festum Domini/i
          || $version !~ /1960/)
      ) {
        push(@collect, "$sortnum;;$sday");
        next;
      }
  ...
  if ($version !~ /1570/i) {
    if ($dayofweek > 2) {
      push(@tfer, sprintf("01-%02i=$tempname/Nat2-0", 8 - $dayofweek));
    } else {
      push(@tfer, "01-02=$tempname/Nat2-0");
    }
  }
  @tfer = sort(@tfer);
