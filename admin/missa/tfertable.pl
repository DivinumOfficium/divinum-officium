...
sub tfertable {
  ...
  #*** save edited value
  $fname =
      ($version =~ /Newcal/i) ? "TrNewcal$kyear"
    : ($version =~ /1955|1960/) ? "Tr1960$kyear"
    : ($version =~ /1570/i) ? "Tr1570$kyear"
    : ($version =~ /1910/) ? "Tr1910$kyear"
    : ($version =~ /Monastic/i) ? "TrM$kyear"
    : "TrDA$kyear";
  $fname1 = $fname;
  $fname1 =~ s/^Tr/Str/;
  ...
  if (do_write("$dfolder/Latin/Tabulae/$fname.txt", @tfer_out)) {
    ...
    $error .= "$dfolder/Latin/Tabulae/$fname.txt cannot open for output<BR>\n";
  }
  ...
  if (do_write("$dfolder/Latin/Tabulae/$fname1.txt", @scriptfer_out)) {
    ...
    $error .= "$dfolder/Latin/Tabulae/$fname1.txt cannot open for output<BR>\n";
  }
}
...
sub collect_arrays {
  ...
  if (@a = do_read("$dfolder/Latin/Tabulae/K$kalendarname.txt")) {
    ...
    error("$dfolder/Latin/Tabulae/$kalendarname.txt cannot open");
  }
  ...
  if ($dayofweek > 2) {
    push(@tfer, sprintf("01-%02i=Tempora/Nat2-0", 8 - $dayofweek));
  } else {
    push(@tfer, "01-02=Tempora/Nat2-0");
  }
  @tfer = sort(@tfer);
