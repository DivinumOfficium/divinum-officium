...
@folders1=(Ordinarium,Psalterium,Tempora,Sancti,Commune,psalms,'psalms1',Tabulae,tones, 
...
if ($folder1) {
  ...
  elsif ($folder1 =~ /tones/i) {$dirname1 = "$datafolder/tones"; $ext1 = "txt";}
  elsif ($folder1 =~ /(Tabulae|Martyrologium1)/i) 
    {$dirname1 = "$datafolder/Latin/$folder1"; $ext1 = 'txt';}
...
if ($lang2 !~ /(none|serach)/i) {
  ...
  if ($folder2 =~ /tones/i) {$dirname2 = "$datafolder/tones"; $ext2 = "txt";}
  elsif ($folder2 =~ /(Tabulae|Martyrologium1)/i) {$dirname2 = "$datafolder/Latin/$folder2"; $ext2 = 'txt';}
  else {$dirname2 = "$datafolder/$lang2/$folder2"; $ext2 = "txt"}   
  ...
  for ($i = 0; $i < @o; $i++) {
    ...
    #hash keys
  	if ($o[$i] =~ /\[([a-z 0-9]+?)\]/i) {  
	    $block = $1;    
      if ($block =~ 
        /(Capitulum|Ant\s+[1-4]|Ant\s+(Prima|Tertia|Sexta|Nona)|Lectio|Responsory|Oratio|Versum)/i) 
        {$mode = $1;}
	    else {$mode = '';}  
      $flag = 1;   
	  }
