...
use DivinumOfficium::Directorium
  qw(get_from_directorium hymnmerge hymnshift hymnshiftmerge);
...
sub invitatorium {
    my $lang = shift;
    my %invit =
      %{ setupstring( $lang, 'Psalterium/Special/Matutinum Special.txt' ) };
    my $name = gettempora('Invitatorium');

    if ( $version =~ /Trid|Monastic/i
        && ( !$name || ( $name eq 'Quad' && $dayofweek != 0 ) ) )
    {
        $name = 'Trid';
    }
    ...
    if (   $i == 0
        && $name =~ /^Invit$/i
        && ( $month < 4 || ( $monthday && $monthday =~ /^1[0-9][0-9]\-/ ) ) )
    {
        $i = 7;
    }
    my @invit = split( "\n", $invit{$name} );
    setbuild( 'Psalterium/Special/Matutinum Special',
        $name, 'Invitatorium ord' );
    my $ant = chompd( $invit[$i] );
    my ( $w, $c );

    if (
           $version =~ /Monastic|Praedicatorum/i
        && $dayofweek
        && $winner =~ /Pasc/
        && $winner !~ /Pasc[07]/
        && $winner !~ /Pasc5-4/
        && !(
               $version =~ /trident|divino/i
            && $dayname[1] =~ /ascensio|pent|joseph/i
        )
      )
    {
    ...
    if ( $lang =~ /gabc/i ) {
        $ant =~ s/;;(.*)$//;
        $invitMode = $1;
    }    # GABC: strip Invit Mode from Antiphone
    $ant = "Ant. $ant";
    ...
    if ( $lang =~ /gabc/i && $invitMode ) {
        $invitpath = "Psalterium/Invitatorium-$invitMode.txt";
    }
    ...
    if ( my @a = do_read($fname) ) {
            ...
            && $version !~ /Praedicatorum/ )
        {
            ...
            && (
                ...
                || ( $dayname[0] =~ /Quad/i && $version =~ /Trident|Monastic/i )
            )
          )
        {
            # old Invitatorium4
...
sub hymnusmatutinum {
    ...
    $name .= checkmtv( $version, \%winner )
      unless ( exists( $winner{'Hymnus Matutinum'} ) );
    ...
    if ($h) {
        ...
        elsif ( hymnmerge( $version, $day, $month, $year, $dioecesis ) )
        {    # if also 2nd Vesper been omitted
            my ( $h1, $c1 ) = getproprium( "$name Vespera", $lang, 1 );
            $h  =~ s/^(v. )//;
            $h1 =~ s/\_(?!.*\_).*/\_\n$h/s
              ; # find the Doxology as last verse since e.g. Venantius(05-18) has a proper one
            $h = $h1;
...
sub nocturn {
    ...
    my $duplexf  = $version =~ /196/
      || ( $duplex > 2 && $rule !~ /Matins simplex/ && $winner !~ /C12/ );
    antetpsalm( \@psalmi_n, $duplexf, $lang );

    # versus can be text or reference (number)
    my (@vs) =
      ( $select[-1] =~ /^\d+$/
        ? ( @{$psalmi}[ $select[-2] ], @{$psalmi}[ $select[-1] ] )
        : ( $select[-2], $select[-1] ) );
    ensure_single_alleluia( \$vs[0], $lang )
      if alleluia_required( $dayname[0], $votive );
    ensure_single_alleluia( \$vs[1], $lang )
      if alleluia_required( $dayname[0], $votive );
    push( @s, "\n", @vs, "\n" );
}
...
sub psalmi_matutinum {
    ...
    my %psalmi =
      %{ setupstring( $lang, 'Psalterium/Psalmi/Psalmi matutinum.txt' ) };
    ...
    if (   $name
        && $version !~ /Trident/i
        && ( $winner =~ /tempora/i || $name eq 'Nat' || $name eq 'Epi' ) )
    {
        if ( $dayofweek == 0 ) {
            foreach my $i ( 1 .. 3 ) {
                ( $psalmi[ ( $i - 1 ) * 5 + 3 ], $psalmi[ ( $i - 1 ) * 5 + 4 ] )
    ...
    if ( $lang =~ /gabc/i ) {
        foreach my $psalmline (@psalmi) {
            my @a = split( ';;', $psalmline )
              ;    # Retrieve psalmtone given behind second ';;'

            if ( @a > 2 ) {
                my $ant0 = chompd( $a[0] );    # Retrieve Antiphon
                my @psalm0 =
                  split( ';', chompd( $a[1] ) );    # Split multiple Psalms
                my $psalmTone = chompd( $a[2] );    # Retrieve PsalmTone

                foreach my $ps0 (@psalm0) {
                    $ps0 =
                      "'$ps0,$psalmTone'";  # combine psalm tone with all psalms
                }
...
# Trident rubrics: Anticipated Sundays (except infra Oct. Epi on Jan 12) are "Simplex", i.e., 1 nocturn with 3 lessions (of the Gospel Homily!)
    if (
        ...
        && !(
               $dayofweek > 0
            && $version =~ /trident/i
            && $winner{Rank} =~ /Dominica (?!infra.*(?:Nat|Epi))/i
        )
      )
    {
        ...
        unless ( exists( $winner{'Ant Matutinum'} ) ) {
            ...
            {
                my $dname =
                  ( $winner{Rank} =~ /Dominica/i ) ? 'Dominica' : 'Feria';
                my @spec = split( "\n", $psalmi{"Pasch Ant $dname"} );
                foreach my $i ( 3, 4, 8, 9, 13, 14 ) {
                    $psalmi[$i] = $spec[$i];
                }
                ...
                && $name =~
                /^(?:Adv|Quad|Pasch)$/i )    # Advent, Quad, and Paschaltide
            {
                foreach my $i ( 1 .. 3 ) {
                    (
                        $psalmi[ ( $i - 1 ) * 5 + 3 ],
   ...
   # we've already returned.
    ...
    {    #??? ex
        if ( $version =~ /196/ && $name eq 'Asc' ) {
            ...
            setbuild( "Psalmi matutinum",
                'Versus ex Pasch Ant Dominica', 'subst' );
        }
...
sub cujus_q {
    ...
    my $j = 0;                                    # "Cujus …, ipse"
    if (/(virgin|vidu[aæ]|poenitentis|pœnitentis|C6|C7)/i) {
        $j += 2 unless /C[2-5]/;
    }                                             # "Cujus …, ipsa"
...
sub get_absolutio_et_benedictiones {
    ...
    if (
        $num
        && (   $rule =~ /9 lectiones/i && ( $version !~ /Monastic/ )
            || $rule =~ /12 lectiones/ )
      )
    {
        ...
        if ( $num == 3 && $winner !~ /12-25/ && $version !~ /Cist/i ) {
            ...
            if ( $w =~ /!(?:Matt|Marc|Luc|Joannes)/ )
            {    # update last benedictio if so
                my @ev9 = split( /\n/, $ben{Evangelica9} );
                $ben[$rpn] = $ev9[0];
                setbuild2( "3rd Noct. B"
                      . ( $rpn + 1 ) . ". : "
                      . beginwith( $ev9[0] ) );
            }
        }
        ...
        if ( $version =~ /Cist/i ) {
                ...
                && $winner{Rank} =~ /\bss?\.|b\./i )
            {
                my @ben3 = split( /\n/, $ben{"Nocturn 3"} );
                $ben[4] =
"$ben{'Rubrica brevis'} Benedictio. $ben3[3 + cujus_q($winner{Rank})]";
            }
            elsif ( $num == 3 ) {
        ...
        my %mariae =
          %{ setupstring( $lang, subdirname( 'Commune', $version ) . "$1.txt" )
          };
        @ben = split( "\n", $mariae{Benedictio} );
        ...
        && $version =~ /Cist/i )
    {
        ...
        if ( $dayofweek == 1 || $dayofweek == 3 ) {
            ...
            $ben[3] =
              "$ben{'Rubrica brevis'} Benedictio. $ben_feria[$dayofweek-1]";
        }

        unshift @ben, $abs[ $num - 1 ];
        setbuild2(
"Special benedictio: Cistercian Ferial Offices in Winter (Feria $dayofweek 3L)"
        );
...
sub lectiones {
    ...
    if ( $rule !~ /Limit.*?Benedictio/i && $version !~ /Cist/i ) {
        ...
        push( @s, "Absolutio. $a[0]", '$Amen' )
          unless $version =~ /^Ordo Praedicatorum/
          || $rule =~ /sine absolutio/i;
    }
    elsif ( $version !~ /Cist/i || $rule =~ /Matutinum Romanum/i ) {
        push( @s, "\$Pater totum secreto" )
          unless $version =~ /Cist/i && $votive =~ /C12/;
    }
    push( @s, "\n" );

    my $rpn =
        ( $num && $rule =~ /12 lectio/ )     ? 4
      : ( $rule         !~ /Lectio brevis/ ) ? 3
      :                                        1;    # readings per nocturn
    $num ||= 1;
...
sub matins_lectio_responsory_alleluia(\$$) {
    ...
    ensure_single_alleluia( \$resp[3], $lang )
      if !( $version =~ /cist/i && $resp[4] =~ /\&Gloria/i );
    ensure_single_alleluia( \$resp[-1], $lang );
    $$r = join( "\n", @resp );
}
...
sub getC10readingname {
    return "Lectio M101"
      if ( $version !~ /196/ && $month == 9 && $day > 8 && $day < 15 );
    my $satnum = floor( ( $day - 1 ) / 7 + 1 );
    $satnum = 4 if ( $satnum == 5 );
    return sprintf( "Lectio M%02i%s",
        $month, ( $version =~ /1963/i ) ? $satnum : '' );
}
...
sub lectio : ScriptFunc {
    ...
    if ( $winner =~ /C12/i ) {
        $ltype1960 = LT1960_DEFAULT;
    }    # Officium parvum B.M.V.

    if ( $ltype1960 == LT1960_SUNDAY && $num == 3 )
    {    # 1960: 3rd reading in a Sunday office
...
# (unless monastic in Nativitytide and Epiphany => prevent the former Octave days of Stephanus, Joannes, Innocents)
            || (
                ...
                && $rank < 2
                && $winner{Rank} !~
                /vigil|(vide|ex) C10/i    #&& $winner  !~ /09-sab-oct/
    ...
    my $nocturn =
      int( ( $num - 1 ) / ( $rule =~ /12 lectiones/i ? 4 : 3 ) ) + 1;

    #prepares for case of homily instead of scripture
    my $homilyflag =
      ( exists( $commemoratio{Lectio1} )
          && $commemoratio{Lectio1} =~
          /\!(Matt|Mark|Marc|Luke|Luc|Joannes|John)\s+[0-9]+\:[0-9]+\-[0-9]+/i )
      ? 1
      : 0;
        ...
        && $dayofweek > 0 )
    {
        ...
        setbuild2("Lectiones I Nocturno de Homilia Dominicæ anticipiata")
          if $num == 1;
        $homilyflag = 7;
        $nocturn    = 0;
    }
    ...
    if ( $nocturn == 1 && $rule =~ /Lectio1 (Oct|Temp)Nat/i ) {
        my %temp;
        setbuild1("Lectiones in I Nocturno ex Tempora Nativitatis")
          if $num == 1;
  ...
  # GitHub3539: in 1960 rubrics, Scripture on Sunday (26-28) comes from Nativity
            %temp = %{
                officestring( $lang,
                    subdirname( 'Sancti', $version ) . "12-25.txt" )
            };
        }
        else {

            my $tfile =
                subdirname( 'Tempora', $version )
              . sprintf( "Nat%02i", $day )
              . ( $version =~ /Trident/i ? "o.txt" : ".txt" );
            ...
            if ( $rule =~ /12 lectiones/ ) {
                my %scrip = %temp;
                $temp{"Lectio$_"} = lectiones_ex3_fiunt4( \%scrip, $_ )
                  foreach ( 1 .. 4 );
    ...
    if (   $nocturn == 1
        && $rule =~ /Lectio1 tempora/i
        && exists( $scriptura{"Lectio1"} ) )
    {
    ...
    if ( $nocturn == 1 && $version !~ /1963/ && $winner !~ /C12/ ) {
        ...
        if ($file) {
            ...
            setbuild( "Lectiones in I Nocturno de Scriptura", $file, "subst" )
              if $num == 1;
        }
    }
...
# proper 1st Nocturn readings, (at least before 1960) the Monday readings are impeded anyway
    ...
    if ( $winner =~ /C12/i ) {
        if ( ( $version =~ /1960/ || ( $winner =~ /Sancti/i && $rank < 2 ) )
            && $num == 4 )
        {
            $num = 3;
        }
...
# in Septuag/Paschaltide, these get the Lessons from the occurent scripture instead (e.g., 04-13)
    ...
    if (
        ...
        && (   ( $num == 4 && !exists( $w{Lectio1} ) )
            || ( $num == 9 && !exists( $w{Lectio10} ) ) )
      )
    {
    ...
    if (
        ...
        && (
            (      $communetype =~ /^ex/i
                && $commune =~ /Tempora/i
                && $rank >
                3 )    # either we have 'ex Tempora' on Duplex majus or higher
            || (
                (
                    $nocturn == 1    # or we are in the first nocturn
                    && $homilyflag ==
                    1                # and there is a homily to be commemorated
      ...
      )
    {
        ...
        $w = $w{"Lectio$num"};
        if ( $w && $num == 1 ) {
            setbuild2("Lectio1-3 from $commune replacing homily");
        }
    }
    ...
    if (
        ...
        && (
            (
                $communetype =~ /^ex/i
                && ( $rank > 3 || ( $version =~ /Cist/i && $rank > 2.2 ) )
            )
      ...
      )
    {
        ...
        if ( $rule =~
qr/in $nocturn Nocturno Lectiones ex (Commune|C\d+[a-z]*) in (\d+) loco/i
          )
        {
            my $loco = $2;
            %com = %{
                setupstring( $lang,
                    subdirname( 'Commune', $version ) . "$1.txt" )
              }
              unless ( $1 eq 'Commune' );
            ...
            $w = $com{$lecnum};
            if ( $w && $num % ( $rule =~ /12 lectiones/i ? 4 : 3 ) == 1 ) {
                setbuild2("Lectio$num ex $commune{Officium}");
            }
        }
    ...
    if (
        ...
        && (
            ( $num < 4
                && exists( $scriptura{"Lectio$num"} )
            )    # for the first nocturn, if their is scripture available
            || (   $num == 4
                && $rule =~ /12 lect/i
                && exists( $scriptura{"Lectio3"} ) )
    ...
    {            # but not in Tridentinum Duplex II. vel I. classis
        ...
        if ( $infile && $winner !~ /C12/ ) {
            %w = resolveitable( \%w, $infile, $lang );
        }
        ...
        if (   $version =~ /monastic/i
            && $rule =~ /12 lectiones/i
            && ( $version !~ /1963/ || $rule =~ /Lectio1 tempora/i ) )
        {
        ...
        && $num == 4
        && exists( $commemoratio{"Lectio$num"} )
        && ( $version =~ /1960/i ) )
    {
    ...
    if ( $version =~ /monastic/i && $num == 3 ) {
        $w = monastic_lectio3( $w, $lang );
    }
    ...
    if ( $commune{Rule} =~ /Special Lectio $num/ ) {
        %mariae =
          %{ setupstring( $lang, subdirname( 'Commune', $version ) . "C10.txt" )
          };
...
# Issue 5027 No. 5: Acknowledgement of this contraction for Tridentine version (trial)
    if (
        ...
        && (   exists( $winner{Lectio93} )
            || exists( $commemoratio{Lectio7} )
            || $homilyflag )
      )
    {
    ...
    if (
        ...
        && (
            ...
            (
                $rule =~ /9 lectio/i && $num == 9 && !(
                    exists( $winner{Responsory9} )
                    || ( $winner{Rank} =~ /Dominica/i && $version !~ /Trid/i )
                )
            )
...
# Monastic: Commemoratio Sancti unless Sunday outranking duplex or Feast of 1st class (or octave) on a Feria
            || (
                ...
                && $version !~ /Cist/i
                && !(
                       ( $rank > 5.5 && $dayofweek && !homilyflag )
                    || ( $winner{Rank} =~ /Dominica/i && $rank > 3 )
                )
            )
        )

        # Simplex: also look when last lectio has been diverged to Lectio 4
        || (   ( $ltype1960 == LT1960_SANCTORAL || $rank < 2 )
            && $winner =~ /Sancti/i
            && $num == 4 )
      )
    {    # 9th lesson diverged to Legend of Commemorated Saint
        ...
        if (
            (
                $w{Rank} =~ /Simplex/i || ( $version =~ /1955/ && $rank == 1.5 )
            )
            && exists( $w{'Lectio94'} )
            && $version !~ /Cist/i
          )
        ...
        $j0 =
          $homilyflag ? 1 : ( $num == 12 ) ? 9 : 7;   # where to look for Homily

        if (
            (
                   $commemoratio =~ /tempora/i
                && $commemoratio !~ /Nat(29|30|31)/i
                || $commemoratio =~ /01\-05\./
            )
...
#  || exists($commemoratio{Lectio1}) removed as it results in a wrong commemoration of Die infra 8vam (e.g., 2024-04-23)
        {
            ...
            if ( $wc && $version =~ /monastic/i && exists( $w{"Responsory$j0"} )
                || exists( $w{"Responsory12C"} ) )
            {

# In M1930, the assigned responsory when a Homily is commemorated only is sometimes different
                $winner{"Responsory$num"} =
                  $w{"Responsory12C"} || $w{"Responsory$j0"};
                $winner2{"Responsory$num"} =
                  $w{"Responsory12C"} || $w{"Responsory$j0"};
            }
            elsif ( !$wc ) {
                $j0 = 1;
                ...
                if (   $wc
                    && $version =~ /monastic/i
                    && exists( $w{"Responsory1"} )
                    || exists( $w{"Responsory12C"} ) )
                {

# In M1930, the assigned responsory when a Homily is commemorated only is sometimes different
                    $winner{"Responsory$num"} =
                      $w{"Responsory12C"} || $w{"Responsory1"};
                    $winner2{"Responsory$num"} =
                      $w{"Responsory12C"} || $w{"Responsory1"};
                }
            }
            ...
            if ($wc) {
                ...
                my @comm = split( "\n", $comm{'Lectio'} );
                $comment =
                    ( $commemoratio{Rank} =~ /Feria/ )    ? $comm[0]
                  : ( $commemoratio       =~ /01\-05\./ ) ? $comm[3]
                  :                                         $comm[1];
        ...
        if ($transfervigil) {
            if ( !( -e "$datafolder/$lang/$transfervigil" ) ) {
                $transfervigil =~ s/v\.txt/\.txt/;
            }
            my %tro = %{ setupstring( $lang, $transfervigil ) };
            if ( exists( $tro{'Lectio Vigilia'} ) ) {
                $w = $tro{'Lectio Vigilia'};
            }
            setbuild2("Lectio ultimo: Commemoratio pro Vigilia transferenda");
        }
        elsif ( $homilyflag == 9
            && !( $version =~ /2020/ && $commemoratio =~ /08-09t/ ) )
        {
            my %tro = ( columnsel($lang) ) ? %commemoratio : %commemoratio2;

            if ( exists( $tro{'Lectio1'} ) )
            {    # should be always true if $homilyflag
                my $trorank = $tro{Rank};
                $trorank =~ s/;;.*//;
                $w = '!'
                  . translate( 'Commemoratio', $lang )
                  . ": $trorank\n"
                  . $tro{'Lectio1'};
            ...
            && $cflag )
        {
            ...
            if ($wc) {
                ...
                if ( $wc !~ /^\!/ )
                {    # add Commemoratio comment if not there already
                    if ( exists( $w{Rank} ) ) {
                        my @wcr = split( ';;', $w{Rank} );
                        $w = '!'
                          . translate( 'Commemoratio', $lang )
                        ...
                        my %comm =
                          %{ setupstring( $lang, 'Psalterium/Comment.txt' ) };
                        my @comm = split( "\n", $comm{'Lectio'} );
                        $comment = $comm[2];
                        $w       = setfont( $redfont, $comment ) . "\n$wc";
                    }
        ...
        if ( $winner{Rank} =~ /Octav.*(Epi|Corp)/i && $w !~ /!.*Vigil/i ) {
            $w = $wo;
        }
        ;    #*** if removed from top
    ...
    if ( $ltype1960 == LT1960_SANCTORAL && $num == 4 ) {
        if ( exists( $winner{'Lectio94'} ) ) {
            ...
            $w = $w{'Lectio94'};
            setbuild2(
                'Last lectio Commemoratio ex Legenda historica (#94/1960)');
    ...
    if ( ( $ltype1960 || ( $winner =~ /Sancti/i && $rank < 2 ) ) && $num > 2 ) {
        $num = 3;
    }
    ...
    unless ( $rule =~ /Limit.*?Benedictio/i
        || exists( $winner{'In Finem Lectio'} ) )
    {
    ...
    if ( !tedeum_required($num)
        || $version =~ /^(?:Monastic|Ordo Praedicatorum)/ )
    {
        my $s;
        $na = $num;
        ...
        if (   $version =~ /1960/
            && $winner =~ /tempora/i
            && $dayofweek == 0
            && $dayname[0] =~ /(Adv|Quad)/i
            && $na == 3 )
        {
            $na = 9;
        }

        if ( contract_scripture( $num, 1 )
            && $version !~ /Monastic|Ordo Praedicatorum/i )
        {
            $na = 3;
        }
        ...
        if ( $version =~ /1955|1960/ && exists( $w{"Responsory$na 1960"} ) ) {
            ...
            $rule =~ /Responsory Feria/i
            || (   $version =~ /1960/
                && $rule =~ /scriptura1960/i
                && !exists( $winner{"Responsory$na"} ) )
          )
        {
            if ( exists( $scriptura{"Responsory$na"} ) ) {
                $s =
                  ( columnsel($lang) )
                  ? $scriptura{"Responsory$na"}
                  : $scriptura2{"Responsory$na"};
            }
            else {
                $s =
                  ( columnsel($lang) )
                  ? $scriptura{"Lectio$na"}
                  : $scriptura2{"Lectio$na"};
            ...
            if (  !$s
                && $version =~ /1960/
                && exists( $scriptura{"Responsory$na 1960"} ) )
            {
                $s =
                  ( columnsel($lang) )
                  ? $scriptura{"Responsory$na 1960"}
                  : $scriptura2{"Responsory$na 1960"};
            }
        }
        else {
...
sub tedeum_required {
    ...
    (    # last lectio?
      ...
      && (
        ...
        || (   $winner =~ /Sancti|Commune/i
            && $dayname[1] !~ /(Vigilia)/ )    # Commune = Votive
        || $rule   =~ /Feria Te Deum/i
        || $winner =~ /Pasc|Nat|C10/
        || ( $winner    =~ /^Tempora/ && $rank > 5 && $dayofweek )
        || (   $version !~ /19(?:55|6)/
            && $winner =~ /Pent01-[56]|Pent02-[1-4]/ )    # Octave CC
        || (   $version =~ /Divino/
            && $winner =~ /Pent02-6|Pent03-[1-5]/ )       # Octave SSCord
      );
}
...
sub gettype1960 {
    ...
    if ( $version =~ /196/ && $votive !~ /(C9|Defunctorum)/i ) {
        if ( $dayname[1] =~ /post Nativitatem/i ) {
            ...
            && (   $dayname[1] =~ /dominica.*?semiduplex/i
                || $winner =~ /Pasc1\-0/i )
          )
        {
            $type = LT1960_SUNDAY;
        ...
        if (
            $rank < 2
            || (   $dayname[1] =~ /(feria|vigilia|die)/i
                && $dayname[1] !~ /infra octavam/i )
          )
        {
            $type = LT1960_FERIAL;
        }
...
sub responsory_gloria {
    ...
    return $w
      if (
        ( $num == 1 && $winner =~ /(?:Adv1|Pasc0)-0/i && $version !~ /cist/i )
...
sub ant_matutinum_paschal {
    ...
    if ( $dayofweek || ( $dayname[0] =~ /Pasc6/ && $version =~ /196/ ) ) {
        ...
        {        # each nocturn under single antiphonas apart Ascension
        ...
        if (   $dayname[0] =~ /Pasc[1-5]/i
            && $dayname[1] =~ /Dominica/
            && $version !~ /Praedicatorum|Monastic/i )
        {
            my %psalmi =
              %{ setupstring( $lang, 'Psalterium/Psalmi/Psalmi matutinum.txt' )
              };
...
sub initiarule {
    ...
    my $initfile =
      get_from_directorium( 'stransfer', $version, $key, $year, $dioecesis );
...
sub resolveitable {
    ...
    if ( $file !~ /\~B$/ || !$initia )
    { # ==> !( ~B && $initia ); unless there is a conflict between a B rule and a initia
        my $replace = $file =~ /\~R$/ ? 1 : 0;    # if we have a ~R(eplace) rule
        $file =~ s/~[ABR]$//
          ; # remove ~A(fter) or ~B(efore) or ~R(eplace) from the end of the string
        @file = split( '~', $file );    # gather the transfered intias
        $lim =
          $rule =~ /12 lect/ ? 4 : 3;    # in general, allow up to 3 transferals
        ...
        { # if we have an (unreplaced) inita on the day already (so we put the transferred afterwards)
            $start =
              ...
              ? ( $rule =~ /12 lect/ ? 4 : 3 )
              : 2
              ; # if we have one transferred place it no. at 3; otherwise at 2&3
        ...
        $i =
          1;   # here $i is used as a counter for how many files are transferred

        while ( @file && $i <= $lim )
        {      # while we have more transferals and stay in the limit
            $file = shift(@file);

            %winit = %{
                setupstring( $lang,
                    subdirname( 'Tempora', $version ) . "$file.txt" )
            };
...
#$w{"Lectio$start"} = $winit{"Lectio$i"};
        ...
        $i = 2
          ; # from here $i is used as a counter for which Lectio$i gets appended
        ...
        if ( @file > 1
            && !( $rule !~ /(9|12) lectiones/i && $winner =~ /Sancti/i ) )
        {
            $lim   = 2;
            $start = 3;
        ...
        while ( $start < 4 )
        {    # first fill the actual day's inita at their appropriate place
...
# already, then read from Monday instead.
        ...
        %w1 = %{ setupstring( $lang,
                subdirname( 'Tempora', $version ) . "$kd.txt" ); };
        setbuild2(
            "subst: Incipit from $s replaced by Monday readings to avoid repeat"
        );
    }
...
# from the Apostel's epistels or revelation, then it is read
        ...
        setbuild2("subs: Incipit from $1 replaced by occuring scripture")
          if $num == 1;
    }
...
sub prevdayl1 {
    ...
    my %w1 =
      %{ setupstring( $lang, subdirname( 'Sancti', $version ) . "$kd.txt" ) };
    my $l = $w1{Lectio1};
    if ( $l =~ /!.*?$s 1:/i ) { return 1; }
    return 0;
}
...
sub getantmatutinum {
    ...
    if ( $version =~ /monastic/i && $winner{Rule} !~ /Matutinum Romanum/i ) {
        ...
        if ( $winner{Rule} =~ /3 lectio/i )
        {    # in Monastic infra Octavam (with Ferial psalms)
            my $i = $dayofweek;
            $i -= 3 if $i > 3;
            if ( $version =~ /cist/i ) { $i = 1; }
            @nocturns = ( $i, 0 )
              ; # Versicle for 1st Nocturn dep. on $dayofweek; No V&R for 2nd Noct.
            $target = 14;
        }
    }
    ...
    if ( @wprop < $target ) {
        foreach my $noc (@nocturns) {
            ...
            push( @w, shift(@wprop) )
              for 1 ..
              $ppN;    # pass-through psalm lines for nocturn if they exist
            last unless $noc;    # for 3 lectio, no versicle to be appended;

            my ( $vers, $cvers ) =
              getproprium( "Nocturn $noc Versum", $lang, 1 );
            my @vers = split( "\n", $vers );
