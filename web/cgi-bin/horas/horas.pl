...
sub horas {
  ...
  if ($version !~ /(Sarum|Monastic|1570|1955|1960|Newcal|Praedicatorum)/i) {
    ante_post('Ante');
  } else {
    $searchind++;
  }
  ...
  if ($version !~ /(Sarum|Monastic|1570|1955|1960|Newcal|Praedicatorum)/) {
    ante_post('Post');
  } else {
    $searchind++;
  }
...
sub Alleluia_ant {
  ...
  if (($full || ($duplex >= 3) || ($version =~ /1960|Newcal|Monastic|Praedicatorum|Sarum/i))) {
    $s .= ", * $s, $s.";
...
sub ant_Benedictus : ScriptFunc {
  ...
  if ($num == 1 && $duplex < 3 && $version !~ /1960|Newcal|Praedicatorum|Sarum/ && $version !~ /monastic/i) { return "Ant. $ant_parts[0]"; }
...
sub getordinarium {
  ...
  elsif ($version =~ /Sarum/i) { $suffix .= "Sarum"; }
