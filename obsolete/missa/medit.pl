...
if (!$folder2) {
  if ($folder2 =~ /program/i) {
    $dirname2 = "$Bin";
    $ext2 = "pl";
  } elsif ($folder2 =~ /Tabulae/i) {
    $dirname2 = "$datafolder/../horas/Latin/$folder2";
    $ext2 = 'txt';
  } else {
...
if ($folder1) {
  if ($folder1 =~ /program/i) {
    ...
    $dirname1 = "$datafolder/../horas/Latin/$folder1";
    $ext1 = 'txt';
  } else {
...
sub adjust {
  ...
  if ($lang =~ /magyar/i && $masschange) { return accents($t); }

  if ($lang =~ /Latin/i && $folder =~ /(tempora|sancti|commune|votive|test)/i && $masschange) { return prefetchL($t); }
...
sub prefetchL {
  ...
  $t =~ s/Per\s+Dóminum[\.]*/\n\$Per Dominum\n/igs;
  $t =~
    s/(Introitus|Oratio|Graduale|Sequentia|Offertorium|Secreta|Communio|Postcommunio|Super populum)[\.]*/\n[$1]\n/ig;
  $t =~ s/Léctio/\[Lectio\]\nLéctio/g;
