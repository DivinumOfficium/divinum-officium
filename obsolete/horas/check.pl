...
sub check {
  ...
  if (@r = do_read("$datafolder/$rname.txt")) {
    ...
    return "$datafolder/ruler.txt cannot open";
  }
  ...
  if ($version =~ /monastic/i && $rulekey =~ /(9|12) lectio/i) {
    if ($allkeys !~ /LectioE/) { $errs .= "Missing LectioE\n"; }
    if ($allkeys !~ /Responsory9/i) { $errs .= "Missing Responsory9\n"; }
  }
  return $errs;
}
