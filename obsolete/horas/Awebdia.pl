...
sub setcell {
  ...
  if ($column == 1) {
    ...
    if ($voicecolumn =~ /chant/i && ($hora !~ /matutinum/i || $chantmatins)) {
      ...
      if ($cellnote) {
        $cell[$searchind][$column] = $cellnote->grid(
          -row => $searchind - 1,
          -column => $column - 1,
          -columnspan => 2,
        );
  ...
  #creates cell
  $cell[$asearchind][$acolumn] = $middleframe->ROText(
    -background => $bgcolor,
    -width => $linewidth,
    -height => $height,
    -borderwidth => 0,
...
sub setcell_rut {
  ...
  while ($after =~ /\{\^(.*?)\,\,(.*?)\^\}/g) {
    ...
    if ($only || $column == $vcol) {
      if ( $str =~ /^[a-z]$/i
        || $str =~ /\%[\s\pL]+\%/i
        || $str =~ /([\!\#]H[iy]mn|\&lectio|\&antiphona_finalis)/i)
      {
        $speecharray[$cellind] .= $str;
      }
...
sub cellout {
  ...
  if ($text =~ /\+/) {
    ...
    my $imgphoto = $cell->Photo(
      -file => "$datafolder/$iname.gif",
      -format => 'gif',
    );
    my $before = $`;
    $text = $';
...
sub cell_position {
  my $ind = shift;
  if ($ind == -1 && $version !~ /(1955|1960)/) { $anteflag = 1; popup("\$Ante", $voicelang); return voiceit("Ante"); }
...
sub voiceit {
  ...
  do {
    ...
    foreach $line (@t) {
      ...
      if ($line =~ /\$/) {
        if ($line =~ /Per Dominum|Per eumdem|Qui cum Patre|Qui vivis|Qui tecum|Tu autem/i) { push(@o, 'jjj'); }
  ...
  if ($hymnflag) {
    ...
    for ($i = 0; $i < @t; $i++) {
      ...
      if ($t[$i] =~ /(Benedictio\.|Absolutio\.|Ant\.|R\.br\.|\*|V\.|R\.|jjj|kkk)/i) { $flag = 0; next; }
      if ($flag && $t[$i] !~ /\~\s*$/) { $t[$i] .= '~'; }
    }
  }
