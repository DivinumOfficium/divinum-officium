...
@tradtable = (
  ...
  'Duplex majus', 'Duplex II. classis', 'Duplex I. classis', 'Duplex I. clasis',
);
@table1960 =
  ('none', 'Commemoratio', 'IV. classis', 'III. classis', 'III. classis', 'II. classis', 'I. classis', 'I. classis');
@newtable = ('none', 'none', 'Optional', 'Memorial', 'Memorial', 'Feast', 'Solemnity', 'Solemnity');
...
if (opendir(DIR, "$datafolder/Latin/Tabulae")) {
  ...
  $error .= "$datafolder/Latin/Tabulae cannot open<BR>";
}
...
if (@a = do_read("$datafolder/Latin/Tabulae/K$year1.txt")) {
  ...
  $error .= "$datafolder/Latin/Tabulae/K$year1.txt cannot open<BR>";
}
...
if ($year2 !~ /none/i) {
  if (@a = do_read("$datafolder/Latin/Tabulae/K$year2.txt")) {
    ...
    $error .= "$datafolder/Latin/Tabulae/K$year2.txt cannot open<BR>";
  }
}
...
for ($i = 0; $i < $mdays[$month - 1]; $i++) {
  ...
  if ($n1 =~ /=/) {
    ...
    $flink1 = "<A HREF=\"edit.pl?folder1=Sancti&filename1=$file1&edit1=on\" " . "TARGET=_new>$file1</A>";
  }
  $n2 = $h2{$key};
  $file2 = $name2 = $flink2 = '-';
...
sub adjustname {
  ...
  while ($name =~ /=([0-9.]+)=/) {
    ...
    my $post = $';
    $class =
        ($year < 1960) ? $tradtable[$trank]
      : ($year < 1970) ? $table1960[$trank]
      : $newtable[$trank];
  ...
  if ($missing) {
    $name .= "<BR><FONT COLOR=red>$missing</FONT>";
  } else {
